...
export default function ProfilePhotoView() {
  ...
  const initialUserInfo = databaseController.getUserInfo();
  const [profilePhotoPath, setProfilePhotoPath] = useState<string | null>(
    initialUserInfo?.profilePhotoPath ?? null
  );
  const [profilePhotoCrop, setProfilePhotoCrop] = useState<{ x: number; y: number; size: number } | null>(
    initialUserInfo?.profilePhotoCrop ?? null
  );
